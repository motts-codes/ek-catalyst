...
export function ProductGallery({
  ...
  // LOCAL PATCH: resync gallery when the variant changes.
  // `images`/`pageInfo` are seeded from props via `useState`, which only reads the initializer
  // on mount. When a variant option changes, the server streams a new `initialImages` prop, but
  // the internal state would otherwise stay stale until a full page refresh remounts this
  // component. Re-seed state (and re-init embla, which caches slide nodes) whenever the incoming
  // image set changes. Keyed on the joined srcs so it only fires on an actual image change.
  const imagesKey = initialImages.map((image) => image.src).join('|');
  ...
  useEffect(() => {
    ...
    // `initialImages`/`initialPageInfo` are captured via `imagesKey`; embla APIs are stable refs.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [imagesKey, emblaApi, emblaThumbsApi]);
